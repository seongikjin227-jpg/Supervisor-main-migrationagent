...
**Supervisor Agent**가 LLM 기반 의사결정으로 3개의 전문 에이전트를 오케스트레이션합니다.
...
┌─────────────────────────────────────────────────────────────────┐
│                      Supervisor Agent                           │
│        (LLM + ReAct 패턴 · LangGraph 상태 머신)                  │
│                                                                 │
│   DB 폴링 → LLM 판단 → Tool 호출 → 대기 → 반복                   │
└──────────────┬──────────────┬──────────────┬────────────────────┘
               │              │              │
               ▼              ▼              ▼
     ┌──────────────┐ ┌──────────────┐ ┌──────────────┐
...
> **역할**: DB의 대기 작업을 주기적으로 폴링하고, LLM이 직접 어떤 에이전트를 실행할지 판단합니다.

### 동작 방식

Supervisor는 단순한 스케줄러가 아닌 **LLM이 직접 도구를 선택하는 ReAct 패턴** 오케스트레이터입니다.

```
1. Poll   — NEXT_MIG_INFO, NEXT_SQL_INFO 전체 대기 작업 목록 조회
2. Decide — LLM이 우선순위·상태·제약 조건을 고려해 어떤 Tool을 호출할지 결정
3. Call   — run_data_migration / run_sql_conversion / run_sql_tuning 중 하나 호출
4. Loop   — 더 이상 처리할 작업이 없을 때까지 2-3 반복
5. Wait   — 5초 대기 후 1로 돌아감
```

### 오케스트레이션 규칙 (LLM 시스템 프롬프트)

| 규칙 | 내용 |
|------|------|
| 재시도 제한 | `RETRY >= 3`인 Migration 작업은 건너뜀 |
| 우선순위 | `PRIORITY` 컬럼 오름차순으로 처리 |
| 이관 한도 | 한 사이클에 Migration 작업은 최대 `PLANNER_MAX_MIG_PER_CYCLE`건 |
| 튜닝 직렬화 | Tuning Agent는 한 사이클에 1건만 dispatch (동시 튜닝 방지) |
| 독립 실행 | SQL Agent는 Migration 완료 여부와 무관하게 독립 실행 |
| 종료 | 처리할 작업이 없으면 대기 상태로 전환 |
...
- **직렬 실행**: Supervisor가 한 사이클에 1건만 dispatch
...
Supervisor는 한 cycle에서 실제 처리한 작업이 있을 때 `AG_AGENT_RUN_METRICS`에 실행 시간을 집계해 저장합니다. `SUPERVISOR_CYCLE`은 poll, LLM 판단, tool 실행 시간을 포함하며 다음 poll까지의 대기 시간은 제외합니다. `DB_MIGRATION`, `SQL_MIGRATION`, `SQL_TUNING`은 각 에이전트 tool이 처리한 job 수와 총 소요 시간을 기록합니다.
...
CREATE INDEX IX_AG_AGENT_RUN_METRICS_01
    ON AG_AGENT_RUN_METRICS (CYCLE_NO, AGENT_NAME);
```
...
| `stage4` | 활성 SQL ID 및 테스트 대상 테이블 기준으로 정리 | 비활성 SQL 또는 테스트 범위 밖 SQL 삭제, `TARGET_TABLE` 보정 |
...
TEST_MAPPING_TABLES=TB_A,TB_B,TB_C
```
...
다만 기본 운영 경로는 `python main.py`입니다. Supervisor는 Migration, SQL Conversion, SQL Tuning을 함께 판단하고 dispatch하므로 전체 파이프라인 운영에는 Supervisor 실행을 우선 사용합니다.
...
└── server/
    ├── agents/
    │   ├── supervisor/            # Supervisor Agent (LangGraph ReAct)
    │   ├── migration/             # Mig Agent
